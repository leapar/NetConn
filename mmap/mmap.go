...
func Map(f *os.File, prot, flags int,name string) (MMap, error) {
	return MapRegion(f, -1, prot, flags, 0,name)
}
...
// If ANON is set in flags, f is ignored.
func MapRegion(f *os.File, length int, prot, flags int, offset int64, name string) (MMap, error) {
	...
	return mmap(length, uintptr(prot), uintptr(flags), fd, offset,name)
}
