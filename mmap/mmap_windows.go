...
func mmap(len int, prot, flags, hfile uintptr, off int64 ,name string) ([]byte, error) {
	...
	var err error
	var mapName *uint16 = nil
	mapName,err = syscall.UTF16PtrFromString(name)
	if err != nil {
		return nil, err
	}
	...
	h, errno := windows.CreateFileMapping(windows.Handle(hfile), nil, flProtect, maxSizeHigh, maxSizeLow, mapName)
	if h == 0 {
